...
class Sns_Producttabs_Block_List extends Mage_Catalog_Block_Product_Abstract{
	...
	public function getProductsOrder($orderby, $isTotal = 0){
		$params = Mage::app()->getRequest()->getParams();
		$options = (!empty($params['options']))?json_decode($params['options']):$this->getConfigObject();
		...
		$order_by = preg_split("/\,/", $options->order_type);
		$cat = array();
		while( count($order_by) ){
			$item = array();
		...
		if ( Mage::registry('current_category') ){
			...
			if( $catids!='' ){
				...
				$category_ids = preg_split("/[,\s\D]+/", $options->product_category);
			}
		...
		if( $catids!='' ){
			...
			$number = ($numberstart==0)?$options->number_per_display:$options->number_per_loadmore;
			$collection->getSelect()->limit( $number, $numberstart );
		} else {
			if ( $options->number_per_display > 0 && $isTotal == 0){
				$collection->setPageSize($options->number_per_display);
			}
		}
		$items = array();
	...
	public function getProductsCat($catid, $isTotal=0){
		...
		$collection = $this->getProductCollection($options->product_order_by);
		...
		$attribute_to_sort = $options->product_order_by;
		switch ($attribute_to_sort){
			case 'name':
			case 'created_at':
			case 'price':
