<?php
//$themeCfg = Mage::helper('mosera/data')->get();
//$themeCfg->getField
class Sns_Mosera_Helper_Data extends Mage_Core_Helper_Abstract {
    public $data;
	...
	public function get($attributes=array()) {
		//Varien_Profiler::start('SNS Mosera');
		$this->data 						= $this->defaults;
		
		$config = array();
		
		foreach(Mage::getStoreConfig("sns_mosera_cfg") as $k => $group){
			$groupName = $k;
		...
		foreach(Mage::getStoreConfig("sns_mosera_sticky") as $k => $group){
			$groupName = $k;
			foreach($group as $key => $value){
				$config[$groupName.'_'.$key] = $value;
			}
		...
		//Varien_Profiler::stop('SNS Mosera');
	...
	public function getImgSize($size) {
		...
		if($size == 'XXL') return array($imgXXL_w, $imgXXL_h / $imgRate);
		return;
		//	$imgSize = Mage::helper('mosera/data')->getImgSize(S);
		//	$imgSize[0], $imgSize[1]
	}
    ...
    public function getCfg($optionString) {
        return Mage::getStoreConfig('sns_mosera_cfg/' . $optionString);
    }
	...
	public function getImgUrl($_content){
		...
		if($img[0] && is_array($img[0])) {
			...
			return Mage::helper('cms')->getBlockTemplateProcessor()->filter('src="{{media url="wysiwyg/mosera/blog/no-image.jpg"}}"');
		}
	}
}
