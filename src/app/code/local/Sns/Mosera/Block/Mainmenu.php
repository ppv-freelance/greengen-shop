<?php
class Sns_Mosera_Block_Mainmenu extends Mage_Catalog_Block_Navigation {
    ...
    protected function _getMenuItemHtml($category, $level = 0, $isLast = false, $isFirst = false) {
		...
		if($level == 0){
			...
			if($catdetail->getData('mosera_title_alias')) {
				$catTitle = $this->escapeHtml($catdetail->getData('mosera_title_alias'));
			} else {
...
//		        }
		...
		$mainmenu =   Mage::helper('mosera/data')->getField('menu_customMainMenu');
        if($mainmenu=="custom"){
            $custom = $this->_getCustomItems(false, $collect_customitems, 'first').$this->_getCustomItems(false, $collect_customitems, 'last');
            return $custom ;
        }
        if($mainmenu=="menu"){
            return $html;
        ...
        if($mainmenu=="all"){
            return $html.$this->_getCustomItems(true, $collect_customitems, 'last');
        }
    } 
