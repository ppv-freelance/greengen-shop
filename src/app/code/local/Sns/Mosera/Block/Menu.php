<?php
class Sns_Mosera_Block_Menu extends Mage_Catalog_Block_Navigation {
    ...
    protected function _getMenuItemHtml($category, $level = 0, $isLast = false, $isFirst = false) {
        ...
        if($level==0){
	        ...
	        if($catdetail->getData('mosera_groupsubcat')) {
	        	$this->_isgroup = true;
	        	$this->_groupitemwidth = $catdetail->getData('mosera_subcat_colw');
	        } else {
	        	$this->_isgroup = false;
	        }
        }
        ...
        if($level==0){
	        $menutypes = $catdetail->getData('mosera_menutype');
	        $rightblock = $this->_getCatBlock($catdetail, 'mosera_block_r');
	        $topblock = $this->_getCatBlock($catdetail, 'mosera_block_t');
	        $bottomblock = $this->_getCatBlock($catdetail, 'mosera_block_b');
	        $hidelink = $this->_getCatBlock($catdetail, 'mosera_menulink');
        ...
        if ($level == 0) {
        	$submenuwidth = $catdetail->getData('mosera_subcat_w');
		...
		if($level == 0){
			...
			$html[] = '<li class="'.$liclass.'">';
	        $html[] = '<a href="'.$url.'" class="'.$linkClass.'" data-hover = "'. $this->escapeHtml($category->getName()) .'">';
            if($catdetail->getData('mosera_label')) $html[] = '<span class="label">' . $catdetail->getData('mosera_label') . '</span>';
            $icon = $this->_getCatBlock($catdetail, 'mosera_custom_icon');
            if($icon){
    ...
    public function getMenuHtml($momenu = false, $level = 0) {
        ...
        $customitems	=	Mage::helper('mosera/data')->getField('menu_customItems');
...
//		        }
		...
		if($this->_isMomenu) $html .= $this->_getCustomItems(true, false, 'last');
		else $html .= $this->_getCustomItems(false, false, 'last');
    ...
    protected function _getCustomItems($momenu = false, $items, $position){
		...
		foreach($items as $menuitem){
			...
			if($menuitem['position'] == $position){
				$html .= '<li class="'.$liClass.'">';
				$html .= 	'<a class="'.$lickClass.'" target="'.$menuitem['target'].'" href="'.$menuitem['link'].'" data-hover="'.$menuitem['title'].'">';
	...
	protected function _toHtml(){
		if (!$this->getTemplate()){
			$this->setTemplate('sns/blocks/menu.phtml');
		}
