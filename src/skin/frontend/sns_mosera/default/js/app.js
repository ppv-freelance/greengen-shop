...
var ProductMediaManager = {
	...
	destroyElevateZoom: function (elZoom) {
		if(elZoom.length) {
	        jQuery('.zoomContainer').remove();
			jQuery('.zoomWrapper').find('div:empty').remove();
	        jQuery('.zoomWrapper:empty').remove();
	        jQuery('.zoomWrapper').parent().find('div:empty').remove();
	        jQuery('.zoomWrapper .zoomWrapper').unwrap();
			if(elZoom.parent().is('.zoomWrapper')) elZoom.unwrap();
	        elZoom.removeData('elevateZoom');
    	}
	},
    
	createElevateZoom: function (elZoom) {
		if(elZoom.length) {
			elZoom.elevateZoom({
    ...
    swapImage: function(targetImage, selectedLabels) {
		...
		var imageGallery = jQuery('#gallery_img_media');
		
		var add2more = true;
		
		imageGallery.find('.item-img').each(function(){
			if(jQuery(this).attr('data-selectedLabels') == selectedLabels) add2more = false;
			//console.log(jQuery(this).data('selectedLabels'));
		});
		//imageGallery.find('.elevatezoom-gallery img').width();
		var item_img = '<div class="item-img" data-selectedLabels="'+selectedLabels+'">';
		item_img += '<a data-zoom-image="'+ targetImage[0].src +'" data-image="'+ targetImage[0].src +'" data-update="" class="elevatezoom-gallery" href="#" title="">';
		item_img += '<img  src="'+targetImage[0].src+'"  alt=""  title="" />';
		item_img += '</a>';
		item_img += '</div>';
		
		if(add2more) {
			//ProductMediaManager.destroyElevateZoom(jQuery("#sns_product_img_media #zoom_img_media"));
			item_img = imageGallery.data('owlCarousel').prepare(item_img);
            imageGallery.data('owlCarousel').add(item_img, 0);
            imageGallery.data('owlCarousel').update();
			ProductMediaManager.createElevateZoom(jQuery("#sns_product_img_media #zoom_img_media"));
		}
		
		var currel = imageGallery.find('[data-selectedLabels="'+selectedLabels+'"] a.elevatezoom-gallery');

		imageGallery.find('.owl-item').each(function(index){
			if(jQuery(this).find('[data-selectedLabels="'+selectedLabels+'"] a.elevatezoom-gallery').length) {
				//imageGallery.data('owlCarousel').goTo(index);
			}
		});
		currel.trigger('click');
    },
...
jQuery(window).load(function() {
    ...
    jQuery(window).resize(function(){
        if( jQuery('body').hasClass('detailpage') ){
        	SnsExtend.waitForFinalEvent(function(){
    			ProductMediaManager.destroyElevateZoom(jQuery("#sns_product_img_media #zoom_img_media"));
    			setTimeout(function(){
                    ProductMediaManager.destroyElevateZoom(jQuery("#sns_product_img_media #zoom_img_media"));
    				ProductMediaManager.createElevateZoom(jQuery("#sns_product_img_media #zoom_img_media"));
